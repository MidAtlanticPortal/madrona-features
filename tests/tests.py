...
from features.registry import register, alternate, edit, edit_form, related, \
    workspace_json, FeatureConfigurationError, enable_sharing, Link, \
    user_sharing_groups, groups_users_sharing_with
from features.models import Feature, PointFeature, LineFeature, PolygonFeature, \
    FeatureCollection, MultiPolygonFeature
from features.forms import FeatureForm
from nursery.jsonutils import srid_to_urn, srid_to_proj
...
from .models import TestMpa, TestForGeoJSON, TestNoGeomFinal, TestNoGeoJSON, \
    ...
    TestMissingCustomShowFeature, TestGetFormClassFeature, \
    TestGetFormClassFailFeature, TestDeleteFeature, TestFolder, \
    GenericLinksTestFeature, CreateFormTestFeature, CreateTestFeature, \
    UpdateFormTestFeature, UpdateTestFeature, LinkTestFeature, \
    OtherGenericLinksTestFeature, LastGenericLinksTestFeature, Shipwreck, \
    Pipeline, TestArray

# used by some of the tests to temporarily create a template file
from nursery.kml.kml import kml_errors
...
class UpdateTest(TestCase):
    ...
    def test_post_not_owner(self):
        other_user = User.objects.create_user('other', 'other@madrona.org',
                                              password='pword')
        self.client.login(username='other', password='pword')
        response = self.client.post(self.instance_url, {
            'name': 'My New Name',
    ...
    def test_cannot_hack_user_field(self):
        ...
        self.assertEqual(response.status_code, 200)
        edited_instance = UpdateTestFeature.objects.get(
            pk=self.test_instance.pk)
...
class LinkViewValidationTest(TestCase):
    ...
    def test_multiple_select_view_requires_instance_argument(self):
        ...
        with self.assertRaises(FeatureConfigurationError):
            link = alternate('test title',
                             'tests.views.invalid_multiple_select_view',
                             select='multiple')
...
class GenericLinksTest(TestCase):
    ...
    def test_generic_links_reused_by_create_link(self):
        ...
        self.assertEqual(
            GenericLinksTestFeature.get_options().get_link("Generic Link"),
            OtherGenericLinksTestFeature.get_options().get_link("Generic Link"))
        self.assertNotEqual(
            OtherGenericLinksTestFeature.get_options().get_link("Generic Link"),
            LastGenericLinksTestFeature.get_options().get_link(
                "Different Name"))
...
class JsonSerializationTest(TestCase):
    ...
    def test_normal_features(self):
        fcdict = \
        [x for x in self.dict['feature-classes'] if x['title'] == 'Shipwreck'][
            0]
    ...
    def test_generic(self):
        linkdict = \
        [x for x in self.dict['generic-links'] if x['title'] == 'Generic Link'][
            0]
        for f in ["tests_genericlinkstestfeature",
                  "tests_othergenericlinkstestfeature"]:
            self.assertTrue(f in linkdict['models'])
        self.assertFalse("features_shipwreck" in linkdict['models'])

    def test_custom_features(self):
        fcdict = [x for x in self.dict['feature-classes'] if
                  x['id'] == 'tests_testmpa'][0]
        for lr in ['self', 'create', 'edit', 'alternate']:
            self.assertTrue(fcdict['link-relations'][lr])

        fcdict = [x for x in self.dict['feature-classes'] if
                  x['title'] == 'LinkTestFeature'][0]
        for lr in ['self', 'create', 'edit', 'alternate']:
            self.assertTrue(fcdict['link-relations'][lr])
        with self.assertRaises(KeyError):
            fcdict['link-relations']['related']
    ...
    def test_collections(self):
        fcdict = \
        [x for x in self.dict['feature-classes'] if x['title'] == 'TestArray'][
            0]
    ...
    def test_must_own_generic(self):
        ...
        linkdict = [x for x in self.dict['generic-links'] if
                    x['title'] == 'Export KML for Owner'][0]
        self.assertTrue('tests_testmpa' in linkdict['models'])
        self.assertTrue('tests_testfolder' in linkdict['models'])
    ...
    def test_must_own_custom(self):
        ...
        fcdict = [x for x in self.dict['feature-classes']
                  if x['id'] == 'tests_testmpa'][0]
        self.assertEquals(fcdict['link-relations']['alternate'][0]['title'],
                          'Export Misc for Owner')
        fcdict_shared = [x for x in self.dict_shared['feature-classes']
                         if x['id'] == 'tests_testmpa'][0]
        with self.assertRaises(KeyError):
    ...
    def test_limit_to_groups(self):
        self.assertEquals(
            len(self.user.groups.filter(name="SuperSpecialTestGroup")), 0)
        ...
        special_group, created = Group.objects.get_or_create(
            name="SuperSpecialTestGroup")
        self.user.groups.add(special_group)
        self.user.save()
        self.assertEquals(
            len(self.user.groups.filter(name="SuperSpecialTestGroup")), 1)

        dict_post = json.loads(workspace_json(self.user, True))
        fcdict = [x for x in dict_post['feature-classes']
                  if x['id'] == 'tests_testmpa'][0]
        self.assertEquals(fcdict['link-relations']['related'][0]['title'],
                          'Habitat Spreadsheet')
...
class CopyTest(TestCase):
    ...
    def test_copy_multiple_and_custom_copy_method(self):
        ...
        self.assertRegexpMatches(response['X-Madrona-Select'],
                                 r'tests_testmpa_\d+ tests_testfolder_\d+')
...
class SpatialTest(TestCase):
    def setUp(self):
        ...
        self.wreck = Shipwreck(user=self.user, name="Nearby Wreck",
                               geometry_final=g3)
        self.wreck.save()

        g2 = GEOSGeometry('SRID=4326;LINESTRING(-120.42 34.37, -121.42 33.37)')
        g2.transform(settings.GEOMETRY_DB_SRID)
        self.pipeline = Pipeline(user=self.user, name="My Pipeline",
                                 geometry_final=g2)
        self.pipeline.save()
...
class CollectionTest(TestCase):
    ...
    def test_add_remove_at_feature_level(self):
        ...
        self.assertRaises(AssertionError, self.mpa3.add_to_collection,
                          self.folder1)
    ...
    def test_feature_set(self):
        ...
        * look recursively through all containers
        ...
        direct_mpa_children = self.folder1.feature_set(recurse=False,
                                                       feature_classes=[
                                                           TestMpa])
        ...
        recursive_mpa_children = self.folder1.feature_set(recurse=True,
                                                          feature_classes=[
                                                              TestMpa])
...
class SharingTestCase(TestCase):
    def setUp(self):
        ...
        self.user1 = User.objects.create_user('user1', 'test@madrona.org',
                                              password=self.password)
        self.user2 = User.objects.create_user('user2', 'test@madrona.org',
                                              password=self.password)
        self.user3 = User.objects.create_user('user3', 'test@madrona.org',
                                              password=self.password)
        ...
        self.mpa1 = TestMpa.objects.create(name='Test_MPA_1', designation='R',
                                           user=self.user1, geometry_final=g1)
        self.mpa1.save()

        self.mpa2 = TestMpa.objects.create(name='Test_MPA_2', designation='R',
                                           user=self.user2, geometry_final=g1)
        self.mpa2.save()

        self.mpa3 = TestMpa.objects.create(name='Test_MPA_3', designation='R',
                                           user=self.user3, geometry_final=g1)
        self.mpa3.save()
        ...
        self.array1 = TestArray.objects.create(name='Test_Array_1',
                                               user=self.user1)
        self.array1.save()
        self.array1.add(self.mpa1)
        self.array1.add(self.pipeline1)

        self.folder1 = TestFolder.objects.create(user=self.user1,
                                                 name="My Folder")
        self.folder1.save()
        self.folder1.add(self.array1)

        self.folder1_share_url = self.folder1.get_options().get_share_form(
            self.folder1.pk)
        self.folder1_resource_url = self.folder1.get_options().get_resource(
            self.folder1.pk)
    ...
    def test_post_share_form(self):
        ...
        self.client.logout()
        self.client.login(username=self.user1.username, password=self.password)
        response = self.client.post(self.folder1_share_url,
                                    {'sharing_groups': [self.group1.pk]})
        self.assertEqual(response.status_code, 200, response.content)
...
class GeoJSONTest(TestCase):
        ...
        '''
         ...
         folder1
          ...
          |- folder2
              | - mpa2
        '''
        self.mpa1 = TestMpa.objects.create(user=self.user, name="Mpa1",
                                           geometry_orig=g1)
        self.mpa2 = TestMpa.objects.create(user=self.user, name="Mpa2",
                                           geometry_orig=g1)
        self.mpa3 = TestMpa.objects.create(user=self.user, name="Mpa3",
                                           geometry_orig=g1)
        self.mpa4 = TestMpa.objects.create(user=self.user2, name="Mpa4",
                                           geometry_orig=g1)
        self.mpa5 = TestForGeoJSON.objects.create(user=self.user, name="Mpa5",
                                                  geometry_orig=g1)
        self.mpa6 = TestNoGeomFinal.objects.create(user=self.user, name="Mpa6")
        self.mpa7 = TestNoGeoJSON.objects.create(user=self.user, name="Mpa7",
                                                 geometry_orig=g1)
    ...
    def test_geojson_byproperty(self):
        ...
        self.assertTrue('application/json' in response['Content-Type'])
        fc = json.loads(response.content)
        self.assertEquals(fc['features'][0]['properties']['absolute_url'],
                          self.mpa5.get_absolute_url())
        self.assertEquals(len(fc['features']), 1)
    ...
    def test_geojson_flat(self):
        ...
        fc = json.loads(response.content)
        self.assertTrue(
            fc['features'][0]['properties']['name'] in ['Mpa1', 'Mpa2'])
        self.assertTrue(
            fc['features'][1]['properties']['name'] in ['Mpa1', 'Mpa2'])
    ...
    def test_geojson_nest_url(self):
        ...
        self.assertTrue('application/json' in response['Content-Type'])
        fc = json.loads(response.content)
        self.assertEquals(fc['features'][0]['properties']['feature_set'],
                          [x.uid for x in [self.mpa1, self.folder2]])
        self.assertEquals(len(fc['features']), 1)
    ...
    def test_geojson_nest_multi(self):
        ...
        link = self.folder1.options.get_link('GeoJSON')
        url = link.reverse(
            [self.mpa1, self.folder2]) + "?strategy=nest_feature_set"
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTrue('application/json' in response['Content-Type'])
        fc = json.loads(response.content)
        self.assertEquals(fc['features'][0]['properties']['name'], 'Mpa1')
        self.assertEquals(fc['features'][1]['properties']['feature_set'],
                          [self.mpa2.uid])
    ...
    def test_geojson_transform(self):
        ...
        response = self.client.get(url)
        fc = json.loads(response.content)
        self.assertEquals(fc['crs']['properties']['name'],
                          srid_to_urn(settings.GEOMETRY_DB_SRID))
        self.assertEquals(
            int(fc['features'][0]['geometry']['coordinates'][0][0][0]), -38619)
        ...
        self.assertEquals(
            int(fc['features'][0]['geometry']['coordinates'][0][0][0]), -120)
        # and this one should be in latlon as well
        settings.GEOJSON_SRID = 4326
        url = link.reverse(
            self.mpa3)  # no need to specify parameter, setting should work
        response = self.client.get(url)
        fc = json.loads(response.content)
        self.assertEquals(fc['crs']['properties']['name'], srid_to_urn(4326))
        self.assertEquals(
    ...
    def test_geojson_download(self):
        ...
        self.assertTrue('attachment; filename=mpa3.geojson' in response[
            'Content-Disposition'])
        # no attach
...
class TestMultiPolygonFeature(TestCase):
    def setUp(self):
        ...
        self.g = GEOSGeometry(
            'SRID=4326;MULTIPOLYGON(((40 40, 20 45, 45 30, 40 40)),((20 35, 45 20, 30 5, 10 10, 10 30, 20 35),(30 20, 20 25, 20 15, 30 20)))')
        self.g.transform(settings.GEOMETRY_DB_SRID)

        self.single_g = GEOSGeometry(
            'SRID=4326;POLYGON((-120.42 34.37, -119.64 34.32, -119.63 34.12, -120.44 34.15, -120.42 34.37))')
        self.single_g.transform(settings.GEOMETRY_DB_SRID)

    def test_feature_type(self):
        mpa = MockMultiPoly(user=self.user, name="My Multi",
                            geometry_orig=self.g)
        mpa.save()
    ...
    def test_single_polygon(self):
        with self.assertRaises(TypeError):
            mpa = MockMultiPoly(user=self.user,
                                name="My Multi with single input",
                                geometry_orig=self.single_g)
            mpa.save()
